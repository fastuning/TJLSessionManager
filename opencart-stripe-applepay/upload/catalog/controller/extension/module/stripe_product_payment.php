...
class ControllerExtensionModuleStripeProductPayment extends Controller {
    ...
    private function debugLog($message) {
        $debug_mode = $this->config->get('payment_stripe_applepay_debug_mode');
        if ($debug_mode) {
            $this->log->write('[Stripe Express Checkout] ' . $message);
        }
    }
    ...
    private function createPaymentIntent($data) {
        ...
        $this->debugLog('createPaymentIntent called with data: ' . print_r($data, true));
        ...
        try {
            ...
            if (isset($payment_intent->id)) {
                ...
                $this->debugLog('ERROR: Failed to create payment intent - Response: ' . print_r($payment_intent, true));
        ...
        try {
            ...
            if (isset($payment_intent->status) && $payment_intent->status == 'succeeded') {
                ...
                if ($order_id) {
                    ...
                    $this->debugLog('confirmPayment - ERROR: Failed to create or');
    ...
    private function getShippingOptions($data) {
        ...
        try {
            ...
            if (!$country_code) {
                $this->debugLog('ERROR: No country code provi');
            ...
            $this->debugLog('Shipping options returned for Spain: 4.95');
    ...
    private function createOrder($product_id, $quantity, $option, $shipping_address, $billing_address, $payment_intent) {
        ...
        $this->debugLog('createOrder - Shipping address received: ' . print_r($shipping_address, true));
        $this->debugLog('createOrder - Billing address received: ' . print_r($billing_address, true));
        ...
        if (empty($address_lines[0]) && !empty($billing_address['address']['line1'])) {
            $this->debugLog('createOrder - Using billing address as fallback for empty shipping addr');
        }
    ...
    private function stripeRequest($endpoint, $data = array(), $method = 'POST') {
        ...
        if (!$secret_key) {
            $this->debugLog('ERROR: No secret key configu');
